...
# Use FindBin::RealBin which works reliably at runtime
our $SCRIPT_DIR;
BEGIN {
    $SCRIPT_DIR = $RealBin;
}
...
# ========================================================================
# ANIMATED SPLASH SCREEN - Fullscreen before creating game windows
# ========================================================================

debug_log("=== SHOWING ANIMATED SPLASH SCREEN ===");

# Load animated ship splash screen frames
my $num_splash_frames = ShipAnimation::load_frames();
debug_log("Loaded $num_splash_frames animation frames");

# Create a function to show the animated splash
sub show_animated_splash {
    # Create fullscreen window for splash
    ...
    # Get first frame
    my $frame = ShipAnimation::get_frame(0);
    my $splash_text = join("\n", @$frame);

    # Create splash viewer
    my $splash_viewer = $splash_win->add(
    ...
    $cui->set_timer(
        ...
        sub {
            ...
            my $frame_text = join("\n", @$frame);
            $splash_viewer->text($frame_text);
            $cui->draw();
            $current_frame = ($current_frame + 1) % $num_splash_frames;
        },
        0.1
    );

    # Bind common keys to dismiss splash
    my $dismiss_splash = sub {
        debug_log("SPLASH: Key pressed, dismissing splash");
        $dismissed = 1;
        debug_log("SPLASH: Clearing timer...");
        eval { $cui->clear_timer('splash_animation'); };
        debug_log("SPLASH: Timer cleared (or already gone)");
        debug_log("SPLASH: Calling mainloopExit...");
        $cui->mainloopExit();
        debug_log("SPLASH: MainloopExit called");
    };

    # Bind multiple keys that might be pressed
    debug_log("SPLASH: Setting up key bindings...");
    foreach my $key (' ', "\n", "\r", 'q', 'Q', "\e", "\cC",
                     'a'..'z', 'A'..'Z', '0'..'9',
                     KEY_ENTER, "\cJ", "\cM") {
        $splash_viewer->set_binding($dismiss_splash, $key);
    }

    # Also set default/catchall binding
    $splash_viewer->set_binding($dismiss_splash, -1);  # -1 = default key handler
    debug_log("SPLASH: Key bindings set");

    $splash_viewer->focus();
    $cui->draw();
    debug_log("SPLASH: Entering splash mainloop...");

    # Run modal loop for splash
    $cui->mainloop();

    debug_log("SPLASH: Exited splash mainloop");

    # Now clean up the splash window
    debug_log("SPLASH: Deleting splash window...");
    eval { $cui->delete('splash_window'); };
    if ($@) {
        debug_log("SPLASH: Error deleting splash window: $@");
    } else {
        debug_log("SPLASH: Splash window deleted successfully");
    }
}

# Show the splash screen and wait for keypress
debug_log("Calling show_animated_splash()...");
show_animated_splash();

debug_log("Splash dismissed, creating game windows...");

# ========================================================================
# GAME WINDOWS - Created after splash is dismissed
# ========================================================================
...
our %player = (
    ...
    ports_visited => 0,  # Number of voyages completed (for new player grace period)
);
...
# True if $value is a non-empty positive integer string (standard input validation for cash/goods amounts)
sub is_valid_positive_amount {
    my ($value) = @_;
    return defined $value && $value ne '' && $value =~ /^\d+$/ && $value > 0;
}

# Units of cargo currently aboard the fleet, summed across all goods
sub cargo_used {
    return $player{cargo}{opium} + $player{cargo}{arms} + $player{cargo}{silk} + $player{cargo}{general};
}

# Free hold space remaining across the whole fleet
sub cargo_free_space {
    return ($player{ships} * $player{hold_capacity}) - cargo_used();
}

# Units currently stored in a port's warehouse, summed across all goods
sub warehouse_used {
    my ($port) = @_;
    my $wh = $warehouses{$port};
    return $wh->{opium} + $wh->{arms} + $wh->{silk} + $wh->{general};
}

# Free space remaining in a port's warehouse
sub warehouse_free_space {
    my ($port) = @_;
    return $warehouses{$port}{capacity} - warehouse_used($port);
}
...
sub load_game {
    ...
    if (defined $filename && $filename ne '') {
        # Reading and parsing the save file can die (missing file, corrupt/incompatible
        # JSON) - wrap it so a bad save shows an error dialog instead of crashing the
        # whole Curses app and leaving the terminal in a broken state.
        eval {
        ...
        if ($@) {
            ...
            $cui->error("Failed to load VOC ledger: $@");
            return;
        }
        ...
        $player{wu_escort} //= 0;
        $player{ports_visited} //= 0;  # Default to 0 for old saves (new player protection)
...
sub advance_date {
    ...
    while ($player{date}{day} > 30) {
        ...
        # NEW PLAYER PROTECTION: No usury until 10 voyages AND debt >= 100k
        if ($player{debt} > 0) {
            ...
            # But give new players a break: no usury until they've sailed 10 times AND debt >= 100k
            if ($player{debt} > ($player{bank_balance} * 10)) {
                if ($player{ports_visited} >= 10 && $player{debt} >= 100000) {
                    $interest_rate = 0.20;  # USURY! 20% per month
                    debug_log("USURY RATES APPLIED: debt=$player{debt} > 10x bank_balance=" . ($player{bank_balance} * 10) . ", voyages=$player{ports_visited}");
                } else {
                    debug_log("USURY WAIVED (new player): debt=$player{debt}, voyages=$player{ports_visited}, threshold not met");
                }
            }
    ...
    # Li Yuen tribute expiry (Book Lines 8363-8368, Line 170)
    # TR = 0 at end of 5% of voyages (protection expires randomly)
    if ($player{li_yuen_tribute} && int(rand(20)) == 0) {  # 5% chance (1-in-20)
        $player{li_yuen_tribute} = 0;
        debug_log("LI YUEN: Tribute expired (TR=0), protection lost");
        # Player will be notified when Li Yuen attacks next
    }
}
...
# Show instructions in the Known World window (animated splash already shown)
...
# Show New/Load game choice (animated splash already shown)
debug_log("Showing New/Load game choice");
    ...
    $text_entry->set_binding( sub {
        ...
        if ($current_action eq 'game_choice') {
            ...
            if (exists $good_map{$letter}) {
                ...
                my $free_space = cargo_free_space();
            ...
            if (exists $good_map{$letter}) {
                ...
                my $wh_remaining = warehouse_free_space($current_port);
...
sub update_status {
    ...
    # Only show if player hasn't already declined retirement
    if ($net_worth >= 1000000 && !$player{retire_offered}) {
...
sub update_hold {
    ...
    $player{cargo}{remaining} = cargo_free_space();

    # Get current port's warehouse
    my $wh = $warehouses{$player{port}};
    my $wh_free = warehouse_free_space($player{port});
...
sub random_event {
    ...
    # NEW PLAYER GRACE PERIOD: Don't send enforcers until player has visited 10 ports
    if ($player{debt} > 0 && $player{debt} > ($player{bank_balance} * 10) && $player{ports_visited} >= 10) {
        ...
        $num_ships = $enforcers;
        debug_log("ENFORCER ATTACK! Debt=$player{debt}, Bank=$player{bank_balance}, Enforcers=$enforcers, ports_visited=$player{ports_visited}");
    ...
    sub listbox_ship_menu_changed {
        ...
        if (defined $selected) {
            if ($selected eq 'Buy Ships') {
                ...
                $prompt_label->text("Guns cost ¥$cost_per_gun_round per gun for all $player{ships} ships. How many guns per ship? [Enter for max: $max_guns] ");
                $text_entry->text('');
                $text_entry->focus();
...
sub check_port_events {
    ...
    # 2. LI YUEN SEA GODDESS DONATION (Book Lines 9168-9213, A31)
    # Triggers in Hong Kong when: cash > 100 AND li_yuen_tribute = 0
    # DN = up to half player's cash
    # Prompt: "SEA GODDESS- (Y) OR (N)"
    if ($port eq 'Hong Kong' && $player{cash} > 100 && !$player{li_yuen_tribute}) {
        my $donation_amount = int(rand($player{cash} / 2));  # DN = up to half cash (Line 820)
        if ($donation_amount < 100) { $donation_amount = 100; }

        debug_log("LI YUEN DONATION OFFER: ¥$donation_amount in Hong Kong (cash=¥$player{cash})");

        my $message = "A well-dressed man in dark silks approaches you on the dock, Taipan.\n\n";
        $message .= "'Greetings. I represent... certain maritime interests.'\n\n";
        $message .= "He glances at your ships.\n\n";
        $message .= "'A donation of ¥$donation_amount to the temple of Tin Hau,\n";
        $message .= "the Sea Goddess, would ensure her blessings upon your voyages.'\n\n";
        $message .= "'Li Yuen himself would smile upon such... generosity.'\n\n";
        $message .= "Donate to the Sea Goddess?";

        my $donate = $cui->dialog(
            -title => "Li Yuen's Representative",
            -message => $message,
            -buttons => ['yes', 'no'],
        );

        if ($donate) {
            # Pay tribute (Book Line AMD: C = C - DN, TR = 1)
            $player{cash} -= $donation_amount;
            $player{li_yuen_tribute} = 1;

            debug_log("LI YUEN: Tribute PAID ¥$donation_amount, TR=1 (protection active)");

            my $thanks = "The man bows deeply.\n\n";
            $thanks .= "'Li Yuen is most pleased, Taipan.\n";
            $thanks .= "You sail under his protection now.'\n\n";
            $thanks .= "He hands you a small jade medallion carved with a sea dragon.\n\n";
            $thanks .= "'Show this to any who question your passage.'";
            ...
            # Refuse tribute
            debug_log("LI YUEN: Tribute REFUSED, TR remains 0 (no protection)");

            my $warning = "The man's expression hardens.\n\n";
            $warning .= "'A pity, Taipan.  The South China Sea can be...'\n";
            $warning .= "He pauses meaningfully.\n\n";
            $warning .= "'...unpredictable for those without friends.'\n\n";
            $warning .= "He melts back into the crowd.";
    ...
    # 3. ROBBERY EVENTS (Original Taipan formulas)
...
sub sail_to {
    ...
    if (grep { $_ eq $new_port } @ports) {
        ...
        # Track total voyages completed (for new player grace period)
        $player{ports_visited}++;
        debug_log("Voyage complete to $new_port, total voyages: $player{ports_visited}");
